...
function findTriggerAction(ticket) {
  ...
  const filtered = actions.filter((a) => {
    ...
    const text = a.description.toLowerCase().trim();
 ...
 * OPENAI
 * ========================= */
function buildN2Prompt(ticket, triggerAction) {
  const textoBase = triggerAction?.description || "Nenhuma análise encontrada.";

  return `
Você é um especialista em transformar análise de suporte do Movidesk em tarefa técnica N2.

IMPORTANTE:
1. Use SOMENTE o texto da análise fornecida abaixo.
2. NÃO misture com outros assuntos do ticket.
3. NÃO use histórico antigo do ticket.
4. NÃO invente informações.
5. Se alguma informação estiver ausente, escreva de forma profissional e neutra.
6. Retorne SOMENTE a tarefa final pronta para copiar e colar.
...
6. O impacto deve ser coerente com o relato.
...
Análise base para gerar a tarefa:
Data: ${triggerAction?.createdDate || "N/A"}
Autor: ${triggerAction?.createdBy || "N/A"}
Tipo: ${triggerAction?.type ?? "N/A"}
Texto:
${textoBase}
`.trim();
}

async function generateN2Task(ticket, triggerAction) {
  const prompt = buildN2Prompt(ticket, triggerAction);
...
async function processTicketJob(job) {
  ...
  try {
    ...
    const n2Text = await retry(
      () => generateN2Task(ticket, triggerAction),
